...
- MUST declare every Mongo index explicitly via `@Indexed` / `@CompoundIndex` on the `@Document` AND apply indexes through a migration tool (`mongock`, `mongobee`) or a startup `IndexOps` job gated behind a profile. NEVER set `spring.data.mongodb.auto-index-creation=true` in `staging` or `prod` — auto-creation hides index drift and surprises ops.
