...
- **FR-003a** *[MUST]* — `TenantId` is an `AggregateId` for the **Tenant aggregate** (`@JvmInline value class TenantId(override val value: UUID) : AggregateId`). It lives at `**/domain/shared/TenantId.kt` — kept in `domain/shared/` (not in `domain/tenant/`) because commands / events across every other aggregate reference it, and the shared package keeps cross-aggregate references clean. **Tenants ARE event-sourced by wallet-accountant** — the Tenant aggregate itself (state, commands like `RegisterTenant`, events like `TenantRegistered`, lifecycle) is defined by a separate PRD (out of scope here). Zitadel Organizations are maintained as a **downstream projection** of the Tenant aggregate's events: an event handler — most likely a Restate workflow per [ADR-001](../../architecture/decisions/ADR-001-axon-5-restate-division-of-labor.md), because calling Zitadel's admin API is an external side effect that needs durable execution — creates / updates / deletes the corresponding Zitadel Organization on `TenantRegistered` / etc. The Tenant aggregate is the **source of truth**; Zitadel is a synchronised replica for authentication purposes. The JWT `tid` claim (per [ADR-005](../../architecture/decisions/ADR-005-oauth-zitadel-local-jwt.md)) still carries the same `TenantId` value — it just originates from our aggregate, not from Zitadel.
...
- [ ] **AC-011**: `TenantId` is a `@JvmInline value class` under `**/domain/shared/` that **implements** `AggregateId`. A static-analysis test (Konsist / ArchUnit) asserts: `TenantId`'s declared supertype includes `AggregateId`; `TenantId` lives at `domain/shared/TenantId.kt` (kept in the shared package because commands / events across every other aggregate reference it); `TenantId` and `AccountId` (and any future `AggregateId` subtypes) are all `@JvmInline value class` over `UUID`. — Verify: Konsist / ArchUnit architectural assertion.
...
6. **`TenantId` placement and parentage** → **`TenantId` is an `AggregateId` of the (separate-PRD) Tenant aggregate.** `@JvmInline value class TenantId(override val value: UUID) : AggregateId` at `**/domain/shared/TenantId.kt` — kept in `domain/shared/` (not in `domain/tenant/`) because commands / events across every other aggregate reference it. **Tenants ARE event-sourced by wallet-accountant**; Zitadel Organizations are maintained as a downstream projection of the Tenant aggregate's events (an event handler, likely a Restate workflow, calls Zitadel's admin API to sync). Our aggregate is the source of truth; Zitadel is a synchronised replica for authentication. The JWT `tid` claim carries the same `TenantId` value — it just originates from our aggregate, not from Zitadel. (FR-003a, AC-011; see the future Tenant-aggregate PRD for the aggregate's shape)
