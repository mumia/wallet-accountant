...
- Every access token MUST carry a `tid` claim whose value equals the `TenantId` of the wallet-accountant Tenant aggregate (per [ADR-002](ADR-002-multi-tenant-isolation.md) and the Tenant-aggregate PRD). Zitadel emits this claim via a Zitadel Action or claim mapper sourced from the Zitadel Organization ID — the Organization ID is kept equal to the `TenantId` by the domain event handler that syncs `TenantRegistered` (and future Tenant-lifecycle events) from wallet-accountant to Zitadel via its admin API. The direction of setup is wallet-accountant → Zitadel; the Tenant aggregate is the source of truth. The Spring Security `JwtAuthenticationConverter` MUST extract `tid` and populate the request-scoped `TenantContext` referenced in ADR-002.
...
source_hash: f8cb27f1658e092431758cb1ec47f28ea532380cc08b74ab56a4d3edf3664d48
directives_hash: d5f12c4b07ad7d668fd3b71a6c215fbe96f451fb9929be6fc0420b4bf4a1a676
